    ...
    match args.command:
        ...
        case "bucket":
            ...
            if args.purge_objects:
                print(
                    f"Bucket: {args.name}, Purged: {purge_bucket(s3_client, args.name)}"
                )
        ...
        case "host":
            ...
            if args.website_configuration is not None:
                if set_bucket_website_policy(
                    s3_client,
                    args.bucket_name,
                    (True if args.website_configuration == "True" else False),
                ):
            ...
            if args.host_static:
                print(
                    static_web_page_file(s3_client, args.bucket_name, args.host_static)
                )
